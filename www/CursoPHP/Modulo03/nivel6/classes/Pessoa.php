...
class Pessoa {
    ...
    public static function getConnection(){
        if(empty(self::$conn)){
            ...
            $user = $ini['user'];
            $pass = $ini['pass'];
            self::$conn = new PDO("pgsql:host={$host}; port=5432; dbname={$name}; user={$user}; password={$pass}");
    ...
    public static function save($pessoa)
    {
        ...
        return $result->execute([
            ':id' => $pessoa['id'], ':nome' => $pessoa['nome'], ':endereco' => $pessoa['endereco'],
            ':bairro' => $pessoa['bairro'], ':telefone' => $pessoa['telefone'], 
